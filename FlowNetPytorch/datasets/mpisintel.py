...
def make_dataset(dataset_dir, split, dataset_type='clean'):
    ...
    for flow_map in sorted(glob.glob(os.path.join(dataset_dir, flow_dir, '*', '*.flo'))):
        ...
        img1 = os.path.join(img_dir, scene_dir,
                            '{}_{:04d}.png'.format(prefix, frame_nb))
        img2 = os.path.join(img_dir, scene_dir,
                            '{}_{:04d}.png'.format(prefix, frame_nb + 1))
...
def mpi_sintel_clean(root, transform=None, target_transform=None,
    ...
    test_dataset = ListDataset(
        root, test_list, transform, target_transform, flow_transforms.CenterCrop((384, 1024)))
...
def mpi_sintel_both(root, transform=None, target_transform=None,
    ...
    test_dataset = ListDataset(root, test_list1 + test_list2, transform,
                               target_transform, flow_transforms.CenterCrop((384, 1024)))
