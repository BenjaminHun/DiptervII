...
class RandomRotate(object):
    ...
    def __call__(self, inputs, target):
        ...
        warped_coords_rot[..., 0] = \
            (np.cos(diff_rad) - 1) * \
            warped_coords[..., 0] + np.sin(diff_rad) * warped_coords[..., 1]
        ...
        target[:, :, 0] = np.cos(
            angle1_rad)*target_[:, :, 0] + np.sin(angle1_rad)*target_[:, :, 1]
        target[:, :, 1] = -np.sin(angle1_rad)*target_[:,
                                                      :, 0] + np.cos(angle1_rad)*target_[:, :, 1]
        return inputs, target
